...
def contact_list(list_id):
  list_id = int(list_id)
  profiles = db.execute(
    "SELECT * FROM profiles JOIN lists_profiles AS lp ON lp.profile_id = profiles.id JOIN lists ON lp.list_id = lists.id WHERE lists.id = ? ORDER BY last_name",
    list_id,
  )
...
def add_contact(profile_id):
  ...
  if request.method == "POST":
    ...
    for list_id in selected_list:
      exist_contact = db.execute(
        "SELECT * FROM lists JOIN lists_profiles ON lists.id = lists_profiles.list_id WHERE lists.id = ? AND lists_profiles.profile_id = ?",
        list_id,
        profile_id,
      )
      if exist_contact:
        ...
        db.execute(
          "INSERT INTO lists_profiles (list_id, profile_id) VALUES (?, ?)",
          list_id,
          profile_id,
        )
    return redirect("/")
...
def edit_contact(profile_id):
  ...
  current_list = db.execute(
    "SELECT list_id FROM lists_profiles WHERE profile_id = ?", profile_id
  )
  ...
  if request.method == "POST":
    ...
    for list_id in current_list_ids - selected_lists:
      db.execute(
        "DELETE FROM lists_profiles WHERE list_id = ? AND profile_id = ?",
        list_id,
        profile_id,
      )
    ...
    return render_template(
      ...
      current_list_ids=current_list_ids,
    )
...
def delete_contact(profile_id):
  user_id = session["id"]
  list_id = db.execute(
    "SELECT id FROM lists JOIN lists_profiles AS lp ON lp.list_id = lists.id WHERE lp.profile_id = ? AND lists.user_id = ?",
    profile_id,
    user_id,
  )
  if list_id:
    ...
    return render_template(
      "error.html", error_message="Couldn't delete the contact"
    )
