...
def index():
  ...
  profiles = db.execute(
    "SELECT * FROM profiles WHERE user_id = ? ORDER BY id DESC", user_id
  )
  if profiles:
...
def add_list():
  if request.method == "POST":
    ...
    db.execute(
      "INSERT INTO lists (user_id, list_name) VALUES (?, ?)", user_id, listname
    )
    return redirect("/")
  else:
...
def edit_list(list_id):
  if request.method == "POST":
    ...
    current_list_name = db.execute(
      "SELECT list_name FROM lists WHERE id = ?", list_id
    )

    if current_list_name:
      current_list_name = current_list_name[0]["list_name"]
      return render_template(
        "edit-list.html", list_id=list_id, current_list_name=current_list_name
      )
    else:
