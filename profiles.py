...
def profile_detail(profile_id):
  ...
  exist_contact = db.execute(
    "SELECT * FROM profiles JOIN lists_profiles AS lp ON lp.profile_id = profiles.id JOIN lists ON lp.list_id = lists.id WHERE profiles.id = ? AND lists.user_id = ?",
    profile_id,
    user_id,
  )
  if profile:
      ...
      if key.startswith("url"):
        ...
        if link_url:
          try:
            ...
            db.execute(
              "INSERT INTO profile_links (profile_id, link_title, link_url, icon) VALUES (?, ?, ?, ?)",
              profile_id,
              title,
              link_url,
              user_favicon,
              ...
              return render_template(
                "error.html", error_message="Invalid URL, please try again"
              )
        else:
          render_template(
            "error.html",
            error_message="URL not provided, please insert a valid URL",
          )
    return redirect("/")
  else:
...
def edit_profile(profile_id):
  ...
  if not profile:
    return render_template(
      "error.html", error_message="Profile dosen't exist, please try again"
    )
  ...
  if request.method == "POST":
        ...
        if link_url:
          try:
            existing_link = db.execute(
              "SELECT * FROM profile_links WHERE link_url = ? AND profile_id = ?",
              link_url,
              profile_id,
            )
            user_favicon = get_favicon_url(link_url)
            ...
            if link_id and existing_link:
              db.execute(
                "UPDATE profile_links SET link_title = ?, link_url = ?, icon = ? WHERE id = ?",
                title,
                link_url,
                user_favicon,
                link_id,
    ...
    return render_template(
      "edit-profile.html", profile=profile[0], profile_id=profile_id, links=links
    )
...
def delete_profile(profile_id):
  if profile_id:
    ...
    return render_template(
      "error.html", error_message="Couldn't delete the profile"
    )
